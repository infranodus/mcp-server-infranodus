/**
 * HTTP Server entry point for the MCP Server
 ...
 * Supports both JWT access tokens (from OAuth flow) and raw API keys.
 */
async function authMiddleware(
	...
	// JWT failed — try treating the token as a raw API key
...
<html>
<head>
	<title>${brand.name} MCP - Authorize</title>
...
<body>
	...
	<form method="POST" action="/oauth/authorize">
		...
		<label for="api_key">${brand.name} API Key</label>
	...
	async (req: Request, res: Response) => {
		try {
			...
			if (grant_type === "authorization_code") {
				...
				// 2. The real authentication is via the API key in the auth code
			...
			if (api_key || (!grant_type && req.body.api_key)) {
				...
				if (!tokenResponse) {
					res.status(401).json({
						error: "invalid_grant",
						error_description: `Invalid ${brand.name} API key`,
					});
					return;
				}
...
// Start the server
app.listen(PORT, () => {
	console.log(`${brand.name} MCP HTTP Server running on port ${PORT}`);
	...
	console.log(`  - ${brand.envPrefix}_API_BASE: ${BRAND_API_BASE}`);
	console.log(
