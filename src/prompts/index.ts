...
2. Draft 0–5 learnings. Each must be: not derivable from the code or docs in a few reads; something that would have saved time if known at the start; verified (it actually worked); about the project, never about me; and preferably an insight that connects things that are not obviously connected, not a bare fact. One statement each, at most two sentences, with at least two [[wikilinked]] entities (file paths, modules, concepts). No secrets, hostnames, env values, or verbatim error output. Zero is a fine answer.
